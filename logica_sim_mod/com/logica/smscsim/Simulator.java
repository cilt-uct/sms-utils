...
/*
 * 20-09-01 ticp@logica.com added support for sending of delivery info 26-09-01
 * ticp@logica.com debug now in a group
 */
...
public class Simulator {
	...
	protected void menu() throws IOException {
		...
		while (keepRunning) {
			...
			System.out.println("- 7 Log to screen "
					+ (displayInfo ? "off" : "on"));
			System.out.println("- 8 Generate MO Messages "
					+ (displayInfo ? "off" : "on"));
	...
	protected void start() throws IOException {
		if (smscListener == null) {
			...
			factory = new SimulatorPDUProcessorFactory(processors,
					messageStore, deliveryInfoSender, users);
	...
	protected void sendMessageLoop() throws IOException {
				...
				for (int i = 0; i < procCount; i++) {
					...
					if (proc.getSystemId().equals(client)) {
						if (proc.isActive()) {
							...
							System.out
									.print("Type the number of times to loop> ");
							String loopNumber = keyboard.readLine();
							System.out
									.print("Type the interval between messages in miliseconds> ");
							String interval = keyboard.readLine();

							for (int h = 0; h < Integer.parseInt(loopNumber); h++) {
								DeliverSM request = new DeliverSM();

								try {
									request.setShortMessage(message);
									request
											.setSourceAddr(new Address(
													"073"
															+ (int) (Math
																	.random() * 10000000)));
					...
					if (proc.getSystemId().equals(client)) {
						if (proc.isActive()) {
							...
							try {
								request.setShortMessage(message);
								request
										.setSourceAddr(new Address("0219813890"));
								proc.serverRequest(request);
								System.out.println("Message sent.");
							} catch (WrongLengthOfStringException e) {
